...
pub const CodingAgentMessage = union(enum) {
    user: ai.UserMessage,
...
pub fn convertToLlmAlloc(allocator: std.mem.Allocator, source: []const CodingAgentMessage) !LlmMessages {
    ...
    return .{
        .messages = try output.toOwnedSlice(allocator),
        .owned_content = try owned_content.toOwnedSlice(allocator),
        .owned_text = try owned_text.toOwnedSlice(allocator),
    };
}
...
// Ported from packages/coding-agent/src/core/messages.ts bashExecutionToText.
test "coding-agent messages format bash execution text for LLM context" {
...
// Ported from packages/coding-agent/src/core/messages.ts custom/summary creators.
test "coding-agent messages create timestamped custom and summary messages" {
...
// Ported from packages/coding-agent/src/core/messages.ts convertToLlm.
test "coding-agent messages convert custom agent messages into LLM user messages" {
    const allocator = std.testing.allocator;
    ...
    const source = [_]CodingAgentMessage{
        .{ .bash_execution = .{
            .command = "ls",
            .output = "a.txt",
            .exit_code = null,
    ...
    try std.testing.expectEqual(@as(usize, 6), converted.messages.len);
